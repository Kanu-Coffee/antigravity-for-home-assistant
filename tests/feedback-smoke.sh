...
TEST_ID="antigravity-ha-feedback-${RANDOM}-$$"
...
report_count() {
  docker exec "${CONTAINER}" /bin/sh -c '
    root=/config/antigravity-workspace/feedback
    if [ ! -d "${root}" ]; then
      printf "0\n"
      exit 0
    fi
...
assert_json 'bug collection result is incomplete' '
  ...
  and (.report_directory | startswith("/config/antigravity-workspace/feedback/"))
' "${BUG_COLLECT}"
...
[[ $(docker exec "${CONTAINER}" stat -c '%a' \
  /config/antigravity-workspace/feedback) == 700 ]] \
...
assert_json 'unauthenticated preview did not return the web fallback' '
  ...
  and .repository == "Kanu-Coffee/antigravity-for-home-assistant"
  and .label == "bug"
  and (.title | startswith("[Bug] "))
  and (.body_file | endswith("/public-report.md"))
  and .fallback.template == "bug_report.yml"
  and (.fallback.url | startswith("https://github.com/Kanu-Coffee/antigravity-for-home-assistant/issues/new?"))
...
assert_json 'Issue Form fallback embedded the long report body' '
  ...
  and (.url | startswith("https://github.com/Kanu-Coffee/antigravity-for-home-assistant/issues/new?"))
...
for timestamp_mode in expired future; do
  ...
  PREVIEW_STATE="/run/antigravity-ha/ha-feedback-previews/${AMBIGUOUS_REPORT_ID}.json"
  [[ $(docker exec "${CONTAINER}" stat -c '%a' \
    /run/antigravity-ha/ha-feedback-previews) == 700 ]] \
...
assert_json 'successful submission receipt is incomplete' '
  .action == "submitted"
  and .issue_number == 42
  and .issue_url == "https://github.com/Kanu-Coffee/antigravity-for-home-assistant/issues/42"
...
assert_json 'security URL was not kept on the private route' '
  ...
  and .private_reporting_url == "https://github.com/Kanu-Coffee/antigravity-for-home-assistant/security/advisories/new"
  and (has("url") | not)
' "${SECURITY_URL}"
