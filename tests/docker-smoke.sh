...
[[ $(docker run --rm --platform linux/amd64 --entrypoint stat "${IMAGE}" \
  -c '%a:%U:%G' /usr/local/share/antigravity-ha/AGENTS.md) == 644:root:root ]] \
...
DEGRADED_OPTIONS='{"authorized_keys":["ssh-ed25519 AAAA invalid-fixture"],"web_terminal_auto_start_antigravity":false,"tmux_session_name":"antigravity-ha-degraded","antigravity_approval_policy":"on-request","antigravity_sandbox_mode":"danger-full-access","log_level":"info"}'
...
docker exec --workdir /config "${PUBLIC_CONTAINER}" \
  ...
  | docker exec --interactive "${PUBLIC_CONTAINER}" jq --exit-status '
      ...
      | contains("http://127.0.0.1:8099/")
        ...
        and contains("stale")
    ' >/dev/null \
  || fail 'antigravity model-visible instructions did not contain the browser and memory user-flow contracts'
...
docker exec "${PUBLIC_CONTAINER}" /bin/sh -c '
  ...
  '\'' >/dev/null
' || fail 'antigravity did not discover the image-managed Playwright stdio MCP'
...
[[ "${antigravity_OUTPUT}" =~ ^antigravity-cli\ [0-9]+\.[0-9]+\.[0-9]+$ ]] \
...
docker exec "${PUBLIC_CONTAINER}" /bin/sh -c \
  'printf "export antigravity_BIN=/tmp/fake-antigravity\n" >> /run/antigravity-ha/runtime.env'
...
[[ $(docker exec "${PUBLIC_CONTAINER}" stat -c '%a' /run/antigravity-ha/runtime.env) == 600 ]]
...
[[ $(docker exec "${PUBLIC_CONTAINER}" stat -c '%a' /data/antigravity/AGENTS.md) == 644 ]]
[[ $(docker exec "${PUBLIC_CONTAINER}" stat -c '%U:%G' /data/antigravity/AGENTS.md) == root:root ]]
docker exec "${PUBLIC_CONTAINER}" test -f /data/antigravity/AGENTS.md
docker exec "${PUBLIC_CONTAINER}" cmp -s \
  /usr/local/share/antigravity-ha/AGENTS.md /data/antigravity/AGENTS.md
docker exec "${PUBLIC_CONTAINER}" grep -Fq 'Run `ha-config-check`' /data/antigravity/AGENTS.md
...
docker exec "${PUBLIC_CONTAINER}" grep -Fq '# preserved-smoke-marker' /data/antigravity/config.toml
docker exec "${PUBLIC_CONTAINER}" grep -Fq '# preserved-agents-smoke-marker' /data/antigravity/AGENTS.md
docker exec "${PUBLIC_CONTAINER}" test -s /data/ssh/ssh_host_rsa_key.pub
[[ $(docker exec "${PUBLIC_CONTAINER}" stat -c '%a' /data/antigravity/config.toml) == 600 ]]
