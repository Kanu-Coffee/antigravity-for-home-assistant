...
RELEASE_IMAGE=${1:-ghcr.io/kanu-coffee/antigravity-for-home-assistant:0.5.0}
CANDIDATE_IMAGE=${2:-antigravity-for-home-assistant:test}
...
printf '%s' \
  "{\"authorized_keys\":[],\"web_terminal_auto_start_antigravity\":false,\"tmux_session_name\":\"antigravity-ha-update-smoke\",\"antigravity_approval_policy\":\"on-request\",\"antigravity_sandbox_mode\":\"danger-full-access\",\"antigravity_user_files_update_mode\":\"preserve\",\"home_assistant_browser_auto_auth\":true,\"home_assistant_browser_token\":\"${BROWSER_OPTION_MARKER}\",\"log_level\":\"info\"}" \
...
docker exec "${RELEASE_CONTAINER}" cmp -s \
  /usr/local/share/antigravity-ha/AGENTS.md /data/antigravity/AGENTS.md \
...
printf '%s\n' "${AUTH_MARKER}" \
  | docker exec --interactive "${RELEASE_CONTAINER}" /bin/sh -c '
      ...
      jq --null-input --arg marker "${marker}" \
        '\''{OPENAI_API_KEY: $marker}'\'' > /data/antigravity/auth.json
    '
docker exec "${RELEASE_CONTAINER}" /bin/sh -c \
  'printf "\n%s\n" "$1" >> /data/antigravity/config.toml' sh "${CONFIG_MARKER}"
docker exec "${RELEASE_CONTAINER}" /bin/sh -c \
  'printf "\n%s\n" "$1" >> /data/antigravity/AGENTS.md' sh "${AGENTS_MARKER}"
docker exec "${RELEASE_CONTAINER}" /bin/sh -c \
  'umask 077; printf "%s\n" "$1" > /data/antigravity/AGENTS.override.md' \
  sh "${AGENTS_OVERRIDE_MARKER}"
printf '%s\n' "${HA_CONFIG_MARKER}" \
  | docker exec --interactive "${RELEASE_CONTAINER}" /bin/sh -c \
    'umask 077; cat > /config/.antigravity-ha-update-smoke-marker'
docker exec "${RELEASE_CONTAINER}" /bin/sh -c '
...
docker exec "${RELEASE_CONTAINER}" jq --exit-status \
  ...
  '.OPENAI_API_KEY == $marker' /data/antigravity/auth.json >/dev/null \
  || fail 'released auth.json marker is not valid JSON'
docker exec "${RELEASE_CONTAINER}" grep -Fxq \
  "${CONFIG_MARKER}" /data/antigravity/config.toml
docker exec "${RELEASE_CONTAINER}" grep -Fxq \
  "${AGENTS_MARKER}" /data/antigravity/AGENTS.md
docker exec "${RELEASE_CONTAINER}" grep -Fxq \
  "${AGENTS_OVERRIDE_MARKER}" /data/antigravity/AGENTS.override.md

CONFIG_HASH_BEFORE=$(container_hash \
  "${RELEASE_CONTAINER}" /data/antigravity/config.toml)
AUTH_HASH_BEFORE=$(container_hash \
  "${RELEASE_CONTAINER}" /data/antigravity/auth.json)
AGENTS_HASH_BEFORE=$(container_hash \
  "${RELEASE_CONTAINER}" /data/antigravity/AGENTS.md)
AGENTS_OVERRIDE_HASH_BEFORE=$(container_hash \
  "${RELEASE_CONTAINER}" /data/antigravity/AGENTS.override.md)
HA_CONFIG_HASH_BEFORE=$(container_hash \
  "${RELEASE_CONTAINER}" /config/.antigravity-ha-update-smoke-marker)
...
[[ $(container_hash "${CANDIDATE_CONTAINER}" /data/antigravity/config.toml) \
  == "${CONFIG_HASH_BEFORE}" ]] \
  || fail 'user antigravity config changed during image update'
[[ $(container_hash "${CANDIDATE_CONTAINER}" /data/antigravity/auth.json) \
  == "${AUTH_HASH_BEFORE}" ]] \
  || fail 'auth.json changed during image update'
[[ $(container_hash "${CANDIDATE_CONTAINER}" /data/antigravity/AGENTS.md) \
  == "${AGENTS_HASH_BEFORE}" ]] \
  || fail 'persistent AGENTS.md changed during image update'
[[ $(container_hash \
    "${CANDIDATE_CONTAINER}" /data/antigravity/AGENTS.override.md) \
  == "${AGENTS_OVERRIDE_HASH_BEFORE}" ]] \
  || fail 'AGENTS.override.md changed during image update'
[[ $(container_hash \
    "${CANDIDATE_CONTAINER}" /config/.antigravity-ha-update-smoke-marker) \
...
docker exec --workdir /config "${CANDIDATE_CONTAINER}" \
  antigravity debug prompt-input 'verify the Home Assistant dashboard' \
  | docker exec --interactive "${CANDIDATE_CONTAINER}" jq --exit-status '
    ...
    ' >/dev/null \
  || fail 'updated image did not expose the canonical 8099 route to antigravity'
docker exec "${CANDIDATE_CONTAINER}" test -f \
  /etc/antigravity/skills/ha-feedback/SKILL.md \
...
[[ $(docker exec "${CANDIDATE_CONTAINER}" stat -c '%a' \
  /data/antigravity/config.toml) == 600 ]]
[[ $(docker exec "${CANDIDATE_CONTAINER}" stat -c '%a' \
  /data/antigravity/auth.json) == 600 ]]
[[ $(docker exec "${CANDIDATE_CONTAINER}" stat -c '%a' \
  /data/antigravity/AGENTS.md) == 644 ]]

docker exec "${CANDIDATE_CONTAINER}" /bin/sh -c '
  antigravity mcp list --json | jq --exit-status '\''
    any(.[];
      .name == "playwright"
      and .enabled == true
...
' || fail 'refresh_all did not install the image default antigravity config'
...
docker exec "${CANDIDATE_CONTAINER}" /bin/sh -c '
  backup=$1
  test "$(stat -c "%a:%U:%G" /data/antigravity/backups)" = 700:root:root
  test "$(stat -c "%a:%U:%G" /data/antigravity/backups/user-files)" \
    = 700:root:root
  ...
  test "$(stat -c "%a:%U:%G" /data/antigravity/.user-files-update-state.json)" \
    = 600:root:root
  test "$(stat -c "%a:%U:%G" /run/antigravity-ha/user-files-update.lock)" \
    = 600:root:root
...
start_app "${CANDIDATE_CONTAINER}" "${CANDIDATE_IMAGE}"
docker exec "${CANDIDATE_CONTAINER}" grep -Fxq \
  "${POST_REFRESH_CONFIG_MARKER}" /data/antigravity/config.toml \
  || fail 'same-version restart repeated the config refresh'
docker exec "${CANDIDATE_CONTAINER}" grep -Fxq \
  "${POST_REFRESH_AGENTS_MARKER}" /data/antigravity/AGENTS.md \
