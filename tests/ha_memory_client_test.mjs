...
const installedModule =
  "file:///usr/local/share/antigravity-ha/ha-memory-ha-client.mjs";
const sourceModule = new URL(
  "../antigravity_home_assistant/rootfs/usr/local/share/antigravity-ha/ha-memory-ha-client.mjs",
  import.meta.url,
).href;
...
if (process.env.HA_MEMORY_INSTALLED_TEST === "1") {
  test("the installed ws transport completes a Supervisor-style snapshot", async (t) => {
    const { WebSocketServer } = await import(
      "/usr/local/lib/antigravity-ha/playwright/node_modules/ws/wrapper.mjs"
    );
