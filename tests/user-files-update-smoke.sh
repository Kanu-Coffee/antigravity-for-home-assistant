...
TEST_ID="antigravity-ha-user-files-${RANDOM}-$$"
...
backup_count() {
  local volume=$1
  run_volume "${volume}" -c '
    if [[ -d /data/antigravity/backups/user-files ]]; then
      find /data/antigravity/backups/user-files -mindepth 1 -maxdepth 1 -type d \
        | wc -l | tr -d " "
    else
      printf "0\n"
    fi
...
APP_VERSION=$(docker run --rm --platform linux/amd64 --entrypoint /bin/sh \
  "${IMAGE}" -c 'cat /usr/local/share/antigravity-ha/app-version')
...
[[ $(docker run --rm --platform linux/amd64 --entrypoint stat "${IMAGE}" \
  -c '%a:%U:%G' /usr/local/share/antigravity-ha/app-version) == 644:root:root ]]
[[ $(docker run --rm --platform linux/amd64 --entrypoint stat "${IMAGE}" \
  -c '%a:%U:%G' /usr/local/share/antigravity-ha/user-files-update.mjs) == 644:root:root ]]

docker run --rm \
  --platform linux/amd64 \
  --entrypoint /usr/local/bin/antigravity-user-files-update \
  "${IMAGE}" unexpected-argument >/dev/null 2>&1 \
  && fail 'user-file update wrapper accepted an argument'

# Missing antigravity_user_files_update_mode must preserve every existing user file.
docker run --rm \
  --platform linux/amd64 \
  --entrypoint /bin/sh \
  ...
  "${IMAGE}" -c '
    ...
    printf "%s\n" \
      "{\"antigravity_approval_policy\":\"never\",\"antigravity_sandbox_mode\":\"workspace-write\"}" \
      > /data/options.json
    printf "# %s\n" "${CONFIG_SECRET}" > /data/antigravity/config.toml
    printf "<!-- %s -->\n" "${AGENTS_SECRET}" > /data/antigravity/AGENTS.md
    printf "{\"token\":\"%s\"}\n" "${AUTH_SECRET}" > /data/antigravity/auth.json
    printf "<!-- %s -->\n" "${OVERRIDE_SECRET}" \
      > /data/antigravity/AGENTS.override.md
    chmod 0600 /data/options.json /data/antigravity/config.toml \
      /data/antigravity/auth.json /data/antigravity/AGENTS.override.md
    chmod 0644 /data/antigravity/AGENTS.md
  '

CONFIG_HASH_BEFORE=$(path_hash "${MAIN_VOLUME}" /data/antigravity/config.toml)
AGENTS_HASH_BEFORE=$(path_hash "${MAIN_VOLUME}" /data/antigravity/AGENTS.md)
AUTH_HASH_BEFORE=$(path_hash "${MAIN_VOLUME}" /data/antigravity/auth.json)
OVERRIDE_HASH_BEFORE=$(path_hash \
  "${MAIN_VOLUME}" /data/antigravity/AGENTS.override.md)
...
[[ $(path_hash "${MAIN_VOLUME}" /data/antigravity/config.toml) \
  == "${CONFIG_HASH_BEFORE}" ]]
[[ $(path_hash "${MAIN_VOLUME}" /data/antigravity/AGENTS.md) \
  == "${AGENTS_HASH_BEFORE}" ]]
[[ $(path_hash "${MAIN_VOLUME}" /data/antigravity/auth.json) \
  == "${AUTH_HASH_BEFORE}" ]]
[[ $(path_hash "${MAIN_VOLUME}" /data/antigravity/AGENTS.override.md) \
  == "${OVERRIDE_HASH_BEFORE}" ]]
run_volume "${MAIN_VOLUME}" -c '
  test ! -e /data/antigravity/.user-files-update-state.json
  test ! -e /data/antigravity/.user-files-update-journal.json
  test ! -e /data/antigravity/backups/user-files
'

# AGENTS-only refresh creates a private exact backup and marks only that scope.
run_volume "${MAIN_VOLUME}" -c '
  jq ".antigravity_user_files_update_mode = \"refresh_agents\"" \
...
run_volume "${MAIN_VOLUME}" -c '
  ...
  test "$(stat -c "%a:%U:%G" /data/antigravity/.user-files-update-state.json)" \
    = 600:root:root
  test "$(stat -c "%a:%U:%G" /data/antigravity/AGENTS.md)" = 644:root:root
  test ! -e /data/antigravity/.user-files-update-journal.json
  cmp -s /usr/local/share/antigravity-ha/AGENTS.md /data/antigravity/AGENTS.md
...
run_volume "${MAIN_VOLUME}" -c \
  'grep -Fq -- "$1" /data/antigravity/AGENTS.md' sh "${POST_AGENTS_MARKER}"
...
run_volume "${MAIN_VOLUME}" -c '
  jq ".antigravity_user_files_update_mode = \"refresh_all\"" \
    /data/options.json > /data/options.json.tmp
  chmod 0600 /data/options.json.tmp
  mv /data/options.json.tmp /data/options.json
'
...
run_volume "${MAIN_VOLUME}" -c '
  ...
  printf '\''approval_policy = "never"\nsandbox_mode = "workspace-write"\ncli_auth_credentials_store = "file"\ncheck_for_update_on_startup = false\n'\'' \
    | cmp -s - /data/antigravity/config.toml
  test "$(stat -c "%a:%U:%G" /data/antigravity/config.toml)" = 600:root:root
  jq --exit-status --arg version "${version}" '\''
...
run_volume "${MAIN_VOLUME}" -c \
  'printf "\n# %s\n" "$1" >> /data/antigravity/config.toml' \
...
assert_json "${CONFIG_REPEAT_OUTPUT}" \
  '.refreshed == [] and .backup_directory == null'
run_volume "${MAIN_VOLUME}" -c \
  'grep -Fq -- "$1" /data/antigravity/config.toml' sh "${POST_CONFIG_MARKER}"
...
run_volume "${MAIN_VOLUME}" -c '
  ...
  '\'' /data/antigravity/.user-files-update-state.json \
    > /data/antigravity/.user-files-update-state.json.tmp
  chmod 0600 /data/antigravity/.user-files-update-state.json.tmp
  mv /data/antigravity/.user-files-update-state.json.tmp \
    /data/antigravity/.user-files-update-state.json
...
docker run --rm \
  ...
  "${IMAGE}" -c '
    ...
    umask 077
    mkdir -p /data/antigravity
    printf "%s\n" '\''{"antigravity_user_files_update_mode":"refresh_agents"}'\'' \
      > /data/options.json
    printf "<!-- %s -->\n" "${AGENTS_SECRET}" > /data/antigravity/AGENTS.md
    chmod 0644 /data/antigravity/AGENTS.md
  '
RECOVERY_ORIGINAL_HASH=$(path_hash \
  "${RECOVERY_VOLUME}" /data/antigravity/AGENTS.md)
...
run_volume "${RECOVERY_VOLUME}" -c '
  ...
  chmod 0600 /data/antigravity/.user-files-update-journal.json
...
[[ $(path_hash "${RECOVERY_VOLUME}" /data/antigravity/AGENTS.md) \
  == "${COMMITTED_EDIT_HASH}" ]] \
  || fail 'committed stale-journal cleanup changed a later user edit'
run_volume "${RECOVERY_VOLUME}" -c '
  test ! -e /data/antigravity/.user-files-update-journal.json
  grep -Fq committed-user-edit /data/antigravity/AGENTS.md
'
...
run_volume "${RECOVERY_VOLUME}" -c '
  ...
  version=$2
  transaction=$3
  cp "${backup}/agents.image-default" /data/antigravity/AGENTS.md
  chmod 0644 /data/antigravity/AGENTS.md
  ...
  mv /data/antigravity/.user-files-update-state.json.tmp \
    /data/antigravity/.user-files-update-state.json
  jq '\''.antigravity_user_files_update_mode = "preserve"'\'' \
    /data/options.json > /data/options.json.tmp
  chmod 0600 /data/options.json.tmp
  mv /data/options.json.tmp /data/options.json
  jq --null-input --arg version "${version}" --arg transaction "${transaction}" '\''
...
docker run --rm \
  ...
  "${IMAGE}" -c '
    ...
    printf "%s\n" '\''{"antigravity_user_files_update_mode":"refresh_all"}'\'' \
      > /data/options.json
    printf "# %s\n" "${CONFIG_SECRET}" > /data/antigravity/config.toml
    printf "<!-- %s -->\n" "${AGENTS_SECRET}" > /data/antigravity/real-agents
    ln -s real-agents /data/antigravity/AGENTS.md
  '
SYMLINK_CONFIG_HASH=$(path_hash \
  "${SYMLINK_VOLUME}" /data/antigravity/config.toml)
set +e
...
[[ $(path_hash "${SYMLINK_VOLUME}" /data/antigravity/config.toml) \
  == "${SYMLINK_CONFIG_HASH}" ]]
run_volume "${SYMLINK_VOLUME}" -c '
  test -L /data/antigravity/AGENTS.md
  test "$(readlink /data/antigravity/AGENTS.md)" = real-agents
  test ! -e /data/antigravity/.user-files-update-state.json
  test ! -e /data/antigravity/.user-files-update-journal.json
  test ! -e /data/antigravity/backups/user-files
'
...
docker run --rm \
  ...
  "${IMAGE}" -c '
    ...
    printf "%s\n" '\''{"antigravity_user_files_update_mode":"refresh_all"}'\'' \
      > /data/options.json
    printf "# %s\n" "${CONFIG_SECRET}" > /data/antigravity/config.toml
    ln /data/antigravity/config.toml /data/antigravity/config-peer
    printf "<!-- %s -->\n" "${AGENTS_SECRET}" > /data/antigravity/AGENTS.md
  '
HARDLINK_CONFIG_HASH=$(path_hash \
  "${HARDLINK_VOLUME}" /data/antigravity/config.toml)
HARDLINK_AGENTS_HASH=$(path_hash \
  "${HARDLINK_VOLUME}" /data/antigravity/AGENTS.md)
set +e
...
[[ $(path_hash "${HARDLINK_VOLUME}" /data/antigravity/config.toml) \
  == "${HARDLINK_CONFIG_HASH}" ]]
[[ $(path_hash "${HARDLINK_VOLUME}" /data/antigravity/AGENTS.md) \
  == "${HARDLINK_AGENTS_HASH}" ]]
run_volume "${HARDLINK_VOLUME}" -c '
  test "$(stat -c "%h" /data/antigravity/config.toml)" = 2
  test ! -e /data/antigravity/.user-files-update-state.json
  test ! -e /data/antigravity/.user-files-update-journal.json
  test ! -e /data/antigravity/backups/user-files
'
...
run_volume "${NONREGULAR_VOLUME}" -c '
  ...
  printf "%s\n" '\''{"antigravity_user_files_update_mode":"refresh_agents"}'\'' \
    > /data/options.json
  mkfifo /data/antigravity/AGENTS.md
'
set +e
...
run_volume "${NONREGULAR_VOLUME}" -c '
  test -p /data/antigravity/AGENTS.md
  test ! -e /data/antigravity/.user-files-update-state.json
  test ! -e /data/antigravity/.user-files-update-journal.json
  test ! -e /data/antigravity/backups/user-files
'
...
LOCK_OUTPUT=$(run_volume "${NONREGULAR_VOLUME}" -c '
  install -d -m 0700 /run/antigravity-ha
  : > /run/antigravity-ha/hardlink-victim
  chmod 0644 /run/antigravity-ha/hardlink-victim
  ln /run/antigravity-ha/hardlink-victim /run/antigravity-ha/user-files-update.lock
  set +e
  /usr/local/bin/antigravity-user-files-update > /tmp/lock-output 2>&1
  status=$?
  set -e
  if [[ $(stat -c "%a" /run/antigravity-ha/hardlink-victim) != 644 ]]; then
