...
def _helper_path(rootfs: Path) -> Path:
    return rootfs / "usr/local/share/antigravity-ha/ha-feedback.mjs"
...
def test_feedback_helper_is_pinned_and_image_managed(
    ...
    assert addon_config["version"] == "2.0.11"
    assert "/feedback/" in (repository_root / ".gitignore").read_text(
        encoding="utf-8"
    )
    dockerfile = (addon_root / "Dockerfile").read_text(encoding="utf-8")
    assert "ARG BUILD_VERSION=2.0.11" in dockerfile
    ...
    assert (
        ...
        and '/usr/bin/node /usr/local/share/antigravity-ha/ha-feedback.mjs "$@"'
        in wrapper
    )
...
def test_issue_forms_keep_manual_and_private_reporting_routes(
    ...
    for form, title, labels, required_ids in expected:
        ...
        assert "antigravity" in route_options
