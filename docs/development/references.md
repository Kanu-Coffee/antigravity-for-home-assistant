...
## OpenAI antigravity 공식

- Antigravity CLI  
  https://developers.openai.com/antigravity/cli

- antigravity authentication 및 headless/device code login  
  https://developers.openai.com/antigravity/auth

- ChatGPT Remote connections — 일반 desktop-host 흐름 참고용이며 이 App의 검증된 모바일 직접 SSH 경로와 구분
  https://developers.openai.com/antigravity/remote-connections

- antigravity configuration reference  
  https://developers.openai.com/antigravity/config-reference

- antigravity non-interactive mode  
  https://developers.openai.com/antigravity/noninteractive

- Antigravity CLI release `0.144.1`
  https://github.com/openai/antigravity/releases/tag/rust-v0.144.1

- antigravity Linux sandbox/container guidance
  https://developers.openai.com/antigravity/agent-approvals-security/

- antigravity environment variables
  https://developers.openai.com/antigravity/environment-variables/

- antigravity custom instructions and `AGENTS.md` discovery
  https://developers.openai.com/antigravity/agent-configuration/agents-md
...
- 이 App의 ChatGPT mobile Remote 경로는 App SSH endpoint에 직접 연결하며, 원격 login shell의 PATH에서 `antigravity`를 찾아 내장 app-server를 bootstrap한다.
- 이 App 경로에는 별도의 Mac/Windows desktop app이나 페어링된 중계 host가 필요하지 않다.
- Headless antigravity는 `antigravity login --device-auth` 또는 local `auth.json` 복사를 지원한다.
- antigravity config는 `approval_policy`, `sandbox_mode`, file credential store를 지원한다.
...
- Antigravity CLI는 `0.144.1` amd64 musl release artifact와 GitHub asset digest를 사용한다.
- 직접 Remote SSH는 remote login shell의 PATH에서 `antigravity`를 찾고 HA App에 저장된 antigravity 인증을 요구한다.
- Supervisor Core/App 로그 endpoint는 `Accept: text/plain` 또는 `text/x-log`를 사용하며 JSON Accept만 보내면 협상이 실패할 수 있다.
- antigravity는 `antigravity_HOME`의 `AGENTS.md`를 전역 지침으로 읽고 프로젝트 root부터 현재 디렉터리까지 더 가까운 지침을 뒤에 결합한다.
