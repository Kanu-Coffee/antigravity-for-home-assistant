...
3. 현재 OpenAI antigravity 설치·인증·Remote SSH 문서 재확인
4. remote가 없으면 private GitHub repo `antigravity-for-home-assistant` 생성
...
## Phase 2 — 영속 디렉터리와 Antigravity CLI

### 구현

- 필수 package 설치
- Antigravity CLI pin 설치
- `/data/home`, `/data/antigravity` 초기화
- 기본 config merge/creation
- `ha-antigravity`, `ha-antigravity-login`
...
일반 shell에서 `/config`를 작업 경로로 antigravity를 실행할 수 있다.
...
HAOS Web UI에서 shell/antigravity가 사용 가능하고 재접속 시 세션이 유지된다.
...
- public key 성공
- host key 재시작 유지
- `ssh host 'command -v antigravity; antigravity --version; pwd'`
...
antigravity가 설정·상태·로그를 분석하고 안전한 실제 동작을 시험할 수 있다.
...
- `/data/antigravity-ha-memory` `0700`, `memory.sqlite3`와 WAL/SHM `0600`
...
- daemon/Core 실패 상태에서도 antigravity, ttyd, SSH, ingress와 browser smoke 성공
...
완료 조건: 검증되지 않은 정보가 active context에 들어가지 않고 antigravity change 결과가 fresh HA evidence로만 반영된다.
...
- 새 기본 `AGENTS.md`에는 규칙·경로만 추가하고 entity별 data 비누적을 명시. `/etc/antigravity/config.toml` developer instruction과 MCP description에도 bounded lookup, empty/degraded/stale 고지, same-request explicit remember와 persistent-change 규칙 제공
...
- `antigravity` wrapper는 user config를 수정하지 않고 server default와 16개 per-tool approval mode를 CLI override로 적용한다. invalid enum/type은 78, init은 fatal이다.
- static parity, fake argv, pinned antigravity parse와 public `0.3.2` update 보존을 자동 검증한다. 동일 merge SHA의 main CI [`29408206017`](https://github.com/Kanu-Coffee/antigravity-for-home-assistant/actions/runs/29408206017)과 tag Builder [`29408467932`](https://github.com/Kanu-Coffee/antigravity-for-home-assistant/actions/runs/29408467932), 정확한 public `0.4.0` image의 browser approval policy/full browser·gateway·Core WebSocket·ttyd·SSH/public `0.3.2` → `0.4.0` update smoke는 **PASS**다.
- `antigravity_approval_policy=never`가 MCP prompt를 전역 승인할 수 있는 precedence와 full-auto가 proxy allowlist/사용자 요청 범위를 넓히지 않는다는 경계를 문서화한다.
...
완료 조건: Home Assistant UI 선택이 새 antigravity session의 현재 허용 도구에 정확히 적용되고 미래·금지 도구는 fail closed하며 사용자 config/AGENTS/identity가 보존된다.
...
- 빌드 가능한 amd64 App 골격
- Antigravity CLI 설치 및 `/data` 영속 config
- ttyd + tmux Ingress
- 공개키 sshd
...
M1 선택: Antigravity CLI `0.144.1`의 `x86_64-unknown-linux-musl` standalone artifact와 고정 SHA-256.
...
SQLite+FTS5를 `/data/antigravity-ha-memory/memory.sqlite3`에 두고 외부 vector/database service를 사용하지 않는다. HA-derived catalog와 user-semantic memory를 논리적으로 분리하고, 전자는 fresh refresh로만 교정하며 후자 mutation만 history-preserving compensating rollback을 제공한다. `ha-memoryd`는 별도 container나 외부 port가 아니라 같은 App의 non-fatal S6 service다.
...
- antigravity 인증 파일을 image에 bake하지 않음
...
- memory 장애를 이유로 ttyd/SSH/antigravity/browser를 실패시키거나 새 host/Ingress port·sidecar·cloud service를 추가하지 않음
