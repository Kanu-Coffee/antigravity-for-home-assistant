...
EXPECTED_HAOS_GATE_TEST_IDS = {
    ...
    for gate in EXPECTED_MANUAL_GATES
}
EXPECTED_HAOS_GATE_ARCHITECTURES = {
    "haos_aarch64_acceptance": ["aarch64"],
    "haos_amd64_acceptance": ["amd64"],
}
HAOS_ACCEPTANCE_CHECKS = {
    "apparmor_enforce_and_secret_denials",
    "config_share_media_preserved",
    "fresh_install_remote_login",
    "home_assistant_operations",
    "legacy_runtime_data_removed",
    "memory_browser_feedback",
    "native_permission_prompt",
    "network_reconnect_and_auth_recovery",
    "no_app_backup_created",
    "restart_and_haos_reboot_reconnect",
    "supervisor_options_normalized",
    "upgrade_2_1_3_factory_reset_once",
}
EXPECTED_HAOS_GATE_CHECKS = {
    gate: HAOS_ACCEPTANCE_CHECKS for gate in EXPECTED_MANUAL_GATES
}
...
EXPECTED_PUBLIC_INSTALL_CHECKS = {
    ...
    "remote_login_and_automatic_start",
    "remote_restart_persistence",
    "retained_operations_memory_browser_feedback",
...
def validate_candidate(candidate: Any) -> dict[str, Any]:
    ...
    require(candidate.get("schema") == "antigravity-ha-release-candidate/v3", "wrong candidate schema")
    version = validate_version(candidate.get("version"))
    require(
        version.split(".", 1)[0] == "3",
        "candidate requires a numeric v3 release",
    )
...
def validate_haos_gate_report(
    ...
    require(
        report.get("previous_release") is None,
        "HAOS acceptance reports do not embed prior-release credentials or identifiers",
    )
    require(
...
def expected_haos_app_version(candidate: dict[str, Any], gate: str) -> str:
    require(gate in EXPECTED_MANUAL_GATES, "unknown HAOS evidence gate")
    return candidate["haos_rehearsal"]["version"]
...
def validate_public_install_report(
    ...
    *,
    ...
    candidate = validate_candidate(candidate_record)
    expected_version = validate_version(version)
    require(
        expected_version.split(".", 1)[0] == "3",
        "public-install acceptance requires a numeric v3 release",
    )
...
def command_candidate_verify(args: argparse.Namespace) -> None:
    candidate = validate_candidate(load_json(args.candidate))
    require(
        candidate["version"] == args.version,
        "candidate version differs from the numeric release",
    )
    require(
        candidate["source_sha"] == args.source_sha,
        "candidate source differs from the annotated tag",
    )
    require(
        candidate["run_id"]
        == validate_run_number(args.run_id, "candidate run ID"),
        "candidate run ID differs from the annotated tag",
    )
    require(
        candidate["run_attempt"]
        == validate_run_number(args.run_attempt, "candidate run attempt"),
        "candidate run attempt differs from the annotated tag",
    )
    images = candidate["images"]
    values = {
...
This is the fail-closed 3.0 release built from `{candidate['source_sha']}`.
...
Read the App documentation before updating. The first 3.0 start permanently removes
the documented App-owned 2.x runtime data without making a backup. `/config`, `/share`,
`/media`, and the Supervisor-owned options file are outside that deletion boundary.
Remote Control replaces Telegram and SSH, requires a new Antigravity login, and starts
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("version", "source-sha", "run-id", "run-attempt"):
        candidate_verify.add_argument(f"--{name}", required=True)
    candidate_verify.add_argument("--github-output", type=Path)
    candidate_verify.set_defaults(handler=command_candidate_verify)
    ...
    public_install_report.add_argument("--candidate", type=Path, required=True)
