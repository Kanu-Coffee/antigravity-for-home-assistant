...
- **amd64** 장치
- 앱 이미지와 antigravity 인증에 필요한 인터넷 연결
- antigravity를 사용할 수 있는 OpenAI/ChatGPT 계정
...
- `/config`에서 실행되는 antigravity CLI
...
- **방법 2 (6자리 PIN 코드)**: 텔레그램 봇에게 로그에 뜬 6자리 핀 번호(예: `702-215`)를 메시지로 전송
- **방법 3 (수동 Chat ID)**: 설정 탭 `telegram_allowed_chat_ids`에 자신의 숫자 Chat ID 직접 등록

### ⏱️ 텔레그램 응답 시간 및 승인 안내
- **간단한 질문/상태 확인**: 약 10~25초 소요 (첫 10초 후 진행 알림 및 `typing` 인디케이터 지속 전송)
- **자동화 생성/엔티티 점검**: 약 30~90초 소요 (도구 호출 및 검증 진행)
- **대화형 승인 (Inline Approval)**: 파일 수정이나 서비스 호출 시 텔레그램에 `[✅ 승인]` / `[❌ 거부]` 인라인 버튼이 표시되며 버튼 클릭으로 즉시 제어 가능
...
[![Home Assistant에 앱 저장소 추가](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2FKanu-Coffee%2Fantigravity-for-home-assistant)
...
3. App store를 새로고침하고 **Antigravity for Home Assistant**를 선택합니다.
...
기본값 `web_terminal_auto_start_antigravity: false`에서는 Bash가 먼저 열립니다. 브라우저를 닫아도 앱이 실행 중이면 같은 `tmux` 세션이 유지되며, 다음 접속에서 이어집니다. 여러 브라우저 탭은 같은 화면과 입력을 공유하므로 신뢰하는 관리자만 사용하세요.
...
인증은 `/data/antigravity`에 저장되어 일반 앱 재시작·업데이트 동안 유지됩니다. `auth.json`은 access token을 포함할 수 있으므로 출력하거나 Git, 이슈, 채팅에 공유하지 마세요.
...
처음에는 기본값을 권장합니다. 설정을 바꾼 뒤에는 앱을 재시작하고, antigravity 관련 정책이나 MCP 도구가 바뀌었다면 실행 중인 antigravity를 종료한 뒤 새 세션을 시작하세요.

| 설정 | 기본값 | 허용값·의미 | 주의 |
| --- | --- | --- | --- |
| `authorized_keys` | `[]` | SSH를 허용할 OpenSSH 공개키 목록 | 비어 있으면 SSH만 비활성화되고 Web UI는 유지됩니다. 개인키를 넣지 마세요. |
| `web_terminal_auto_start_antigravity` | `false` | 새 `tmux` 세션에서 antigravity를 한 번 자동 시작 | 기존 세션에는 소급되지 않습니다. antigravity 종료 후 Bash로 돌아옵니다. |
| `tmux_session_name` | `antigravity-ha` | 영문, 숫자, `.`, `_`, `-`로 된 1~64자 세션 이름 | 바꾼 뒤 새 세션을 사용합니다. |
| `antigravity_approval_policy` | `on-request` | `untrusted`, `on-request`, `never` | `never`는 광범위한 자동 승인을 뜻하므로 신뢰하는 작업에서만 사용하세요. |
| `antigravity_sandbox_mode` | `danger-full-access` | `workspace-write`, `danger-full-access` | 앱 컨테이너 내부 정책입니다. 그러나 `/config`가 RW이므로 실제 HA 설정에 영향을 줄 수 있습니다. |
| `browser_approval_policy` | `safe` | `safe`, `never`, `always` | `safe`는 조회·캡처를 자동 승인하고 클릭·입력은 확인합니다. |
| `antigravity_user_files_update_mode` | `preserve` | `preserve`, `refresh_agents`, `refresh_all` | 일회성 refresh 뒤 `preserve`로 되돌리세요. `refresh_all`은 사용자 antigravity 설정을 초기화할 수 있습니다. |
...
상위 `antigravity_approval_policy: never`는 antigravity 전체 full-auto 정책이므로 `browser_approval_policy: safe` 또는 `always`의 확인보다 우선할 수 있습니다. 브라우저 정책만으로 보안 경계를 대신하지 마세요.

### antigravity 사용자 파일 갱신

| 값 | 동작 |
| --- | --- |
| `preserve` | 기존 `/data/antigravity/config.toml`과 base `AGENTS.md`를 보존합니다. 기본값입니다. |
...
`refresh_all`은 사용자가 추가한 model, provider, MCP와 기타 설정을 제거할 수 있습니다. 원본은 `/data/antigravity/backups/user-files` 아래 root-only backup에 저장되지만 이 backup 자체도 credential과 내부 endpoint를 포함할 수 있는 민감자료입니다. 작업을 확인한 뒤 mode를 `preserve`로 되돌리지 않으면 다음 앱 버전에서 선택한 갱신이 다시 한 번 실행됩니다.
...
2. **설정 → Apps → Antigravity for Home Assistant → OPEN WEB UI**를 엽니다.
...
ChatGPT 모바일 Remote는 HA 앱의 공개키 SSH endpoint에 직접 연결합니다. HA 앱 이미지에 antigravity CLI와 Remote용 app-server가 모두 포함되어 있으므로 별도의 Mac/Windows 데스크톱 앱이나 중계 호스트가 필요하지 않습니다.
...
5. 연결하면 모바일 Remote가 SSH login shell의 `antigravity`를 사용해 앱 내부 app-server를 시작하고 `/config`에서 antigravity 작업을 엽니다.

일반 SSH client로 접속하면 자동으로 antigravity 화면이 열리는 것이 아니라 `/config`의 Bash shell이 열립니다. 이 경우 `ha-antigravity` 또는 `antigravity`를 직접 실행하세요. 모바일 Remote는 원격 app-server를 자동으로 bootstrap하므로 antigravity 작업 화면으로 바로 이어집니다. Web UI의 공유 `tmux`와 모바일 Remote 세션은 기본적으로 서로 다른 세션입니다.
...
새 antigravity 세션은 image-managed Playwright 도구를 자동으로 사용합니다. 별도 browser package나 MCP를 설치할 필요가 없습니다.

- Home Assistant dashboard는 antigravity 내부의 `http://127.0.0.1:8099` gateway를 사용합니다.
...
`ha_memory`는 이 프로젝트가 구현한 로컬 SQLite/MCP 기능이며 OpenAI antigravity의 Memories와 다릅니다.
...
이 기능은 모델 자체가 스스로 학습하거나 승인 없이 집을 운영한다는 뜻이 아닙니다. 현재 `0.9.7`은 experimental이며 실제 HAOS의 자연어 기억→새 작업 회상 전체 흐름에는 아직 공개 검증 공백이 있습니다.

## 앱 버그·기능 제안 보고서

`0.9.7`부터 image-managed `$ha-feedback` Skill이 앱 자체의 버그와 기능 제안을 읽기 전용으로 조사하고 정제된 보고서를 만듭니다.
...
- 보안 취약점 가능성이 있으면 공개 이슈 검색·미리보기·제출을 중단하고 [비공개 취약점 제보](https://github.com/Kanu-Coffee/antigravity-for-home-assistant/security/advisories/new)를 사용합니다.
...
GitHub 대상은 `Kanu-Coffee/antigravity-for-home-assistant`로 고정됩니다. 로그인은 선택 사항입니다.
...
1. 가능하면 Home Assistant backup을 만들고 진행 중인 antigravity 작업을 마칩니다.
2. App store 저장소를 새로고침하고 일반 **Update**를 사용합니다.
3. 앱을 완전 삭제하거나 `/data`를 초기화하지 마세요.
4. 업데이트 후 앱을 시작하고 기존 antigravity 세션을 종료한 뒤 새 세션을 엽니다.
5. `antigravity login status`, 필요한 MCP와 주요 설정을 확인합니다.

일반 업데이트는 `/data`의 antigravity 인증·설정, SSH host key, 검증형 memory와 선택형 `/data/github-cli` 로그인을 보존하도록 설계되어 있습니다. `antigravity_user_files_update_mode: preserve`가 기본값입니다. `refresh_agents` 또는 `refresh_all`을 유지한 채 다음 버전으로 올리면 선택한 파일이 새 버전에서 한 번 다시 갱신되므로, 일회성 갱신 뒤에는 `preserve`로 되돌리세요.
...
| 명령 | 기능 |
| --- | --- |
| `ha-antigravity` | `/config`에서 antigravity 시작 |
| `ha-antigravity-login` | device code 로그인 시작 |
...
- 세션을 초기화하면 실행 중인 antigravity와 명령이 종료됩니다. 정말 필요한 경우에만 실행합니다.
...
- `antigravity login status`를 확인합니다.
- 계정 또는 workspace에서 device code 로그인이 허용되는지 확인합니다.
- 앱의 `auth.json` 내용을 출력하지 마세요.
- 최신 공식 절차는 [antigravity authentication](https://developers.openai.com/antigravity/auth)을 확인하세요.

### SSH 연결 실패

- `authorized_keys`에 public key 한 줄이 정확히 저장됐는지 확인합니다.
- App Network의 host port와 `~/.ssh/config`의 `Port`가 같은지 확인합니다.
- 먼저 `ssh -v antigravity-ha`의 host, port, 선택된 key만 확인하고 공유 전 개인 경로·주소를 가립니다.
...
- 앱과 기존 antigravity/browser 세션을 다시 시작합니다.
...
1. 관련 antigravity 작업과 자동화를 중지합니다.
...
2. 앱과 기존 browser/antigravity 세션을 재시작합니다.
3. 상태를 확인한 뒤 제거합니다.
...
앱 삭제 전에는 필요한 `/data`의 antigravity 설정·인증, memory와 SSH identity를 어떻게 처리할지 결정하세요. backup에는 credential과 민감한 Home Assistant 맥락이 포함될 수 있습니다.
...
- 실제 HAOS의 `0.9.7` 자연어 memory 폐루프에는 공개 검증 공백이 있습니다.
...
지원 요청 전 [SUPPORT.md](../SUPPORT.md)에 따라 token, 내부 URL, entity·사용자 정보를 제거하세요. 일반 문제는 [GitHub Issues](https://github.com/Kanu-Coffee/antigravity-for-home-assistant/issues), 보안 취약점은 [SECURITY.md](../.github/SECURITY.md)의 비공개 경로를 사용하세요.
