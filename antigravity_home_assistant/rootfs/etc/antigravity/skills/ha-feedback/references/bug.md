...
1. Confirm that the affected surface belongs to Antigravity for Home Assistant. Separate App behavior from Home Assistant Core, Supervisor, another integration, and user configuration; record an uncertain boundary as unverified instead of assigning blame.
2. Collect only the helper's allowlisted App, antigravity, Core, Supervisor, OS, architecture, and safe App-option summary. Do not open raw option, auth, storage, database, backup, log, or screenshot sources.
