...
const TARGET_REPOSITORY = "Kanu-Coffee/antigravity-for-home-assistant";
const PRIVATE_VULNERABILITY_URL =
  "https://github.com/Kanu-Coffee/antigravity-for-home-assistant/security/advisories/new";
const DEFAULT_REPORT_ROOT = "/config/antigravity-workspace/feedback";
const DEFAULT_GH_CONFIG_DIR = "/data/github-cli";
const DEFAULT_GH_BIN = "/usr/local/bin/gh";
const DEFAULT_PREVIEW_ROOT = "/run/antigravity-ha/ha-feedback-previews";
const SAFE_OPTIONS_PATH = "/run/antigravity-ha/ha-feedback-options.json";
...
function normalizeEnvironment(value = {}) {
  ...
  return {
    ...
    antigravity_version: normalizeVersion(
      value.antigravity_version,
      "environment.antigravity_version",
    ),
...
function readantigravityVersion() {
  try {
    ...
    const match = /^antigravity-cli\s+([A-Za-z0-9._+-]+)$/u.exec(output);
...
async function collectEnvironment(fallback) {
  ...
  return normalizeEnvironment({
    ...
    antigravity_version: readantigravityVersion() || fallback.antigravity_version,
...
function renderReport(report) {
  const environmentRows = [
    ...
    ["antigravity", report.environment.antigravity_version],
...
function webFallback(resolved) {
  ...
  const parameters = new URLSearchParams({
    ...
    verification_route:
      "antigravity 검증 Skill 완료 / antigravity verification Skill completed",
  });
