...
const DEFAULT_AGENTS_PATH = "/usr/local/share/antigravity-ha/AGENTS.md";
...
function parseOptions(value) {
  ...
  const mode = value.antigravity_user_files_update_mode ?? "preserve";
  if (typeof mode !== "string" || !VALID_MODES.has(mode)) {
    throw new Error("antigravity_user_files_update_mode is invalid");
  }
  const approvalPolicy = value.antigravity_approval_policy ?? "on-request";
  if (!new Set(["untrusted", "on-request", "never"]).has(approvalPolicy)) {
    throw new Error("antigravity_approval_policy is invalid");
  }
  const sandboxMode = value.antigravity_sandbox_mode ?? "danger-full-access";
