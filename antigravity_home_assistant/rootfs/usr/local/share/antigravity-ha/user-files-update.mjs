...
// Keep the exact public 2.0.11-2.0.18 native layout immutable so preserve mode
// can prove ownership and retire it transactionally. The 2.1 policy itself is
// generated by telegram-permission-policy.mjs for the selected mode.
const LEGACY_2_0_11_2_0_18_NATIVE_READ_PERMISSION_RULES = [
  "read_file(/config)",
...
const LEGACY_2_0_11_2_0_18_SENSITIVE_DENY_PERMISSION_RULES = [
...
const LEGACY_2_0_11_2_0_18_PERMISSION_RULES = {
  allow: [
    ...LEGACY_2_0_11_2_0_18_NATIVE_READ_PERMISSION_RULES,
    ...SAFE_MCP_PERMISSION_RULES.filter(
      (rule) =>
        rule !== "mcp(ha_read/ha_read_addon_logs)" &&
        rule !== "mcp(ha_read/ha_read_host_logs)" &&
        rule !== "mcp(ha_read/ha_read_supervisor_logs)",
    ),
    ...PLAYWRIGHT_READ_ONLY_TOOLS.map((tool) => `mcp(playwright/${tool})`),
  ],
  ask: [],
  deny: [...LEGACY_2_0_11_2_0_18_SENSITIVE_DENY_PERMISSION_RULES],
};
const REQUEST_REVIEW_PERMISSION_RULES =
  canonicalTelegramPermissionRules("request-review");
const ALWAYS_PROCEED_PERMISSION_RULES =
...
const MANAGED_PERMISSION_RULES = new Set([
  ...REQUEST_REVIEW_PERMISSION_RULES.allow,
...
const RETIRED_MANAGED_PERMISSION_RULES = new Set([
  ...
  // 2.0.11-2.0.18 removed the broad native grants and used an exact
  // proposal/read-only allowlist. 2.1 restores broad operational access while
  // retaining proposal-first review when request-review is selected.
  ...LEGACY_2_0_11_2_0_18_PERMISSION_RULES.allow,
  ...LEGACY_2_0_11_2_0_18_PERMISSION_RULES.ask,
  ...LEGACY_2_0_11_2_0_18_PERMISSION_RULES.deny,
...
function parseOptions(value) {
  ...
  if (new Set(["strict", "proceed-in-sandbox"]).has(toolPermission)) {
    const previousToolPermission = toolPermission;
    toolPermission = "request-review";
    migrationWarnings.push(
      `Legacy antigravity_tool_permission=${previousToolPermission} was normalized to request-review because the mode is not supported by this App`,
    );
  }
...
function permissionMigrationSource(ownership, desiredOwnership) {
  ...
  if (
    ...
    return {
      label: "2.0.11-2.0.18",
      rules: LEGACY_2_0_11_2_0_18_PERMISSION_RULES,
    };
  }
  for (const [label, rules] of [
    ["2.1 request-review", REQUEST_REVIEW_PERMISSION_RULES],
