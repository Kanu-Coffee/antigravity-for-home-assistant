...
import WebSocket from "/usr/local/lib/antigravity-ha/playwright/node_modules/ws/wrapper.mjs";

const SUPERVISOR_WEBSOCKET_URL = "ws://supervisor/core/websocket";
const AUTH_STATUS_PATH = "/run/antigravity-ha/browser-auth-status.json";
const BROWSER_TOKEN_PATH = "/run/antigravity-ha/home-assistant-browser.token";
...
const MANAGED_CLIENT_NAME_PREFIX = "Antigravity for Home Assistant browser";
const MANAGED_DISPLAY_NAME_PREFIX = "antigravity Browser (managed)";
  ...
  if (
    ...
    (state.temporary_username !== undefined &&
      (!validOpaqueId(state.temporary_username) ||
        !state.temporary_username.startsWith("antigravity-browser-")))
  ) {
    throw setupError(
      "managed_state_invalid",
...
async function setupManagedBrowserAuth(session, supervisorAccessToken) {
  ...
  try {
    ...
    temporaryUsername = `antigravity-browser-${randomBytes(18).toString("hex")}`;
