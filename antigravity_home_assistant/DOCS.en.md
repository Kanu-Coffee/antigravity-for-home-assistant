...
This guide applies to app version `0.8.0`.
...
This feature does not mean the model trains itself or operates your home without approval. Version `0.8.0` remains experimental, and the complete natural-language memory-to-recall flow has not yet been publicly validated on real HAOS hardware.

## App bug and feature reports

Starting with `0.8.0`, the image-managed `$ha-feedback` Skill investigates app bugs and feature ideas in read-only mode and creates a sanitized report.
...
- Public validation of the complete `0.8.0` natural-language memory loop on real HAOS hardware is still incomplete.
